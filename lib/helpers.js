...
//TODO Make this accept any path even absolute paths possibly
function loadTemplate(templatePath =  'templates/template.html') {
    try {
        return fs.readFileSync(path.resolve(__dirname, templatePath)).toString()
    }
    catch (err) {
        console.error(`Error sourcing the Template : \n${err}`);
        console.info(`Please make sure "${path.basename(__filename, path.extname(__filename))}" is properly installed.`);
    }
}

function selectTemplate(templateName = 'html') {
    switch(templateName){
        case 'html': 
            return  loadTemplate('templates/template.html')
        case 'txt':
        default: 
            return loadTemplate('templates/template.txt')
    }
}
